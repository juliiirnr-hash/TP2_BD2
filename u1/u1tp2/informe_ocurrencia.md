...
DUIA ESCENARIO 1 — "Lectura no repetible"

Herramienta:

OpenCode (modelo/proveedor configurado)
...
SELECT precio FROM producto WHERE id\_producto = 1;



Pestaña 2: UPDATE producto
...
Pestaña 1: precio = 800

(dato actualizado en pestaña 2)
...
Pestaña 1: precio = 500.00

(dato original, no cambió)



Pregunta:

¿Por qué ocurrió esto? (DBeaver - PostgreSQL)
...
Se repitió el experimento en el motor real:



Sin REPEATABLE READ, el segundo SELECT devolvió 800

(dato actualizado).



Con BEGIN ISOLATION LEVEL REPEATABLE READ, el segundo SELECT

devolvió 500.00 (dato original).



Ambos resultados confirmaron la explicación de la IA.

============================================================

DUIA ESCENARIO 2 — "Espera por bloqueo"
...
Tabla completa de id\_producto.



Salida 2:

Se queda colgado (Executing Query...).
...
Name | Query | Value | commit

(confirmación de que se realizó el COMMIT)
...
Se ejecutó FOR UPDATE en Pestaña 1 sin COMMIT:



Pestaña 1 bloqueó la fila.

Pestaña 2 quedó esperando indefinidamente.



Al ejecutar COMMIT en Pestaña 1:



El lock se liberó.

Pestaña 2 se destrabó inmediatamente.

Pestaña 2 devolvió la fila.



Esto confirma que el lock se libera al cerrar la transacción
...
DUIA ESCENARIO 3 — "Lectura fantasma"
...
(estado, forma\_pago, total, id\_usuario)

VALUES

('PENDIENTE', 'EFECTIVO', 100, 1);
...
Se ejecutó el COUNT antes (1) y después (2) del INSERT + COMMIT

de la otra sesión, dentro de la misma transacción con BEGIN

explícito en READ COMMITTED.
...
INSERT + COMMIT en Pestaña 2

Segundo COUNT: 2

El resultado confirmó la lectura fantasma y permitió detectar

el error puntual de la IA sobre la causa del fenómeno.
